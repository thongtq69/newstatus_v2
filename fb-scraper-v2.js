...
function log(msg) {
  const line = `[${now()}] ${msg}`;
  console.log(line);
  fs.appendFileSync(path.join(ROOT, 'scraper-v2.log'), line + '\n');
}
...
async function main() {
  log('========================================');
  log(`FB SCRAPER V2 — batched concurrency ${BATCH_SIZE} | interval: ${CYCLE_INTERVAL_MS / 1000}s | restart every ${BROWSER_RESTART_MS / 60000}min`);
  ...
  log('🔐 Kiểm tra đăng nhập Facebook (V2)...');
  let loggedIn = await checkLogin(loginPage);
  while (!loggedIn) {
    log('⚠️ CHƯA ĐĂNG NHẬP V2! Hãy đăng nhập Facebook trên trình duyệt vừa mở.');
    await waitForEnter('👉 Đăng nhập + xác minh 2FA xong, nhấn ENTER để tiếp tục...');
    loggedIn = await checkLogin(loginPage);
    if (!loggedIn) log('❌ Vẫn chưa đăng nhập V2. Thử lại...');
  }
  log('✅ Đã đăng nhập Facebook V2! Bắt đầu scrape...');

  while (true) {
    if (Date.now() - browserStartTime >= BROWSER_RESTART_MS) {
      log('🔄 Hourly browser restart V2...');
      ...
      log('✅ Browser V2 restarted');
    }

    cycleNum++;
    try { await runCycle(context, pages, coreScript, cycleNum); }
    catch (err) { log(`V2 CYCLE ERROR: ${err.message}`); }
